...
trait FakeTestData {
  ...
  def createFakeUsers(liveMeeting: LiveMeeting): Unit = {
    ...
    val guestWait1 = GuestWaiting(guest1.intId, guest1.htmlName, guest1.role, guest1.guest, "", guest1.authed, System.currentTimeMillis())
    ...
    val guestWait2 = GuestWaiting(guest2.intId, guest2.htmlName, guest2.role, guest2.guest, "", guest2.authed, System.currentTimeMillis())
  ...
  def createFakeUser(liveMeeting: LiveMeeting, regUser: RegisteredUser): UserState = {
    UserState(intId = regUser.id, extId = regUser.externId, plainName = regUser.plainFullname, htmlName = regUser.htmlFullname, role = regUser.role, pin = false,
